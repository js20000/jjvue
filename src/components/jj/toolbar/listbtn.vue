<template>
  ...
  <div v-else>
    <el-button-group v-if="data.editIndex==data.index">
      <jj-button v-for="(btn) in data.column.data.buttons " v-if="btn.state&&btn.state=='edit'" :key="btn.event"
                 :btn="btn" :row="data.row"
                 @click="trigger(btn)"/>
    </el-button-group>
    <div v-else-if="data.editIndex<0">
      <el-button-group>
        <jj-button v-for="(btn) in data.column.data.buttons " v-if="!btn.state||btn.state=='normal'" :key="btn.event"
                   :btn="btn" :row="data.row"
                   @click="trigger(btn)"/>
      </el-button-group>
